...
void handleScript() {
  server.send(200, "application/javascript", "async function load(url, retry) { try { retry = retry || 5; let x = await fetch(url); let t = await x.text(); if (t != null) return t; else throw 'retry'; } catch(e) {return load(url, retry -1);}}; async function loadNr(url, retry) {let x = await load(url, retry); let r = parseInt(x); if (isNaN(r)) return await loadNr(url, (retry || 5) - 1); else return r}; async function loadReg(addr, reg, retry) {if (retry <= 0) return -1; retry = retry || 5; let x = await fetch(\"/read?addr=\" + addr + \"&reg=\" + reg + \"&len=1\"); if (x.status == 200) {let t = await x.text(); if (t != null && t.indexOf('dec: ') > -1) { let valS = t.substr(t.indexOf(\"dec:\") + 5); let val = parseInt(valS); return val; } else return loadReg(addr, reg, retry - 1);} else {return loadReg(addr, reg, retry - 1);}} async function debug() {let html=\"<table><thead><tr><td>Addr</td><td>Val (dec)<td></tr></thead><tbody>\"; let s = document.getElementById('debugOut'); let e = document.getElementById('debugAddress'); let a = e.value; let ret = {}; for (let i = 0; i <= 254; i++) { s.innerText = 'Loading register ' + i; ret[i] = await loadReg(a, i); html += \"<tr><td>\" + i + \"</td><td>\" + ret[i] + \"</td></tr>\";}; ret['errorRatio'] = await load('/modbusErrorRatio'); ;html +=\"</tbody></table>Base64: \" + btoa(JSON.stringify(ret)); s.innerHTML = html;}");
}
